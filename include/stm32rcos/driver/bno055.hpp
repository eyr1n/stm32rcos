...
public:
  ...
  std::optional<Eigen::Quaternionf> get_quaternion() {
    ...
    if (!read_register(0x20, reinterpret_cast<uint8_t *>(data.data()), 8)) {
      return std::nullopt;
    }
  ...
  bool write_register(uint8_t addr, const uint8_t *data, uint8_t size) {
  ...
  bool read_register(uint8_t addr, uint8_t *data, uint8_t size) {
