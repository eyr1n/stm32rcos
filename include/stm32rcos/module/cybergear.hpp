...
enum class CybergearMotorMode : uint8_t {
  NONE = 0,
  RAD = 1,
  RADPS = 2,
  CURRENT = 3,
...
enum class CybergearTurnMode {
  SINGLE_TURN,
  MULTI_TURN,
};

template <class CAN_> class Cybergear : public EncoderBase {
public:
  Cybergear(CAN_ &can, uint8_t motor_id, uint8_t master_id,
  ...
  void init() {
    reset();
    set_motor_mode(CybergearMotorMode::CURRENT);
    enable();
  }
  ...
  bool update() {
    ...
    switch (turnmode_) {
    case CybergearTurnMode::SINGLE_TURN: {
      set_count(count);
      break;
    }
    case CybergearTurnMode::MULTI_TURN: {
      if (delta > (get_cpr() / 2)) {
        delta -= get_cpr();
  ...
  void set_motor_mode(CybergearMotorMode mode) {
    if (mode == CybergearMotorMode::NONE)
      return;
