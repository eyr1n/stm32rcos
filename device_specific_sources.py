...
def get_defines(src):
    ...
    return sorted(set(re.findall(r"^\s*#define\s+(\w+).*$", headers, re.MULTILINE)))
...
{declare_contexts("hcan", get_matched_defines(defines, r"^CAN[0-9]*$"))}
  std::unreachable();
}}
...
{declare_contexts("hfdcan", get_matched_defines(defines, r"^FDCAN[0-9]*$"))}
  std::unreachable();
}}
...
{declare_contexts("hspi", get_matched_defines(defines, r"^SPI[0-9]*$"))}
  std::unreachable();
}}
...
{declare_contexts("htim", get_matched_defines(defines, r"^TIM[0-9]*$"))}
  std::unreachable();
}}
...
{declare_contexts("huart", get_matched_defines(defines, r"^US?ART[0-9]*$"))}
  std::unreachable();
}}
